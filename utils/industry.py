...
def complete_industry_info(input_file, output_file=None):
    ...
    for code in tqdm(stock_codes, desc="获取行业信息"):
        try:
            # 直接使用原始股票代码查询
            data = pro.stock_basic(ts_code=code, fields="ts_code,industry")
            if not data.empty:
                industry_mapping[code] = data.iloc[0]['industry']
...
#数据引入
    ...
    # # 文件路径配置
    # input_file="../data_processed/CSI100/NS_edge/news_20220328.csv"
    # output_file="../data_processed/CSI100/NS_edge/news_20220328_industry.csv"
