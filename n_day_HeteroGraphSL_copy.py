...
# 定义数据集保存路径
DATASET_PATH = "7_day_100_hetero_graph_dataset.pt"
...
def create_daily_graph(date, n_days=7):
    ...
    # 节点文件路径（只使用当天）
    node_file = f"./data_processed/CSI100/7_day_nodes/news_{date}.csv"
    ...
    ss_edge_dfs = []
    for d in date_list:
        ss_file = f"./data_processed/CSI100/SS_edge/news_{d}.csv"
    ...
    ns_is_edge_dfs = []
    for d in date_list:
        ns_file = f"./data_processed/all/2022/NS_IS_NI_edge/news_{d}_with_MR_with_SA_NI.csv"
    ...
    # 添加NI边
    ni_edge_src, ni_edge_dst, ni_edge_attr = [], [], []
    ...
    if not ns_is_edge_df.empty:
        for _, row in ns_is_edge_df.iterrows():
            ...
            if stock_code in stock_map:
                ...
                # N-I边
                if news_id in news_map:
                    ni_edge_src.append(news_map[news_id])
                    ni_edge_dst.append(industry_map[industry_code])
                    ni_edge_attr.append(row['NI_edge'])
    ...
    if ni_edge_src:
        data['news','NI_edge','industry'].edge_index = torch.tensor([ni_edge_src, ni_edge_dst], dtype=torch.long)
        data['news','NI_edge','industry'].edge_attr = torch.tensor(ni_edge_attr, dtype=torch.float)
    
    # 打印统计信息
    print(f"日期 {date} (合并{n_days}天数据):")
    print(f"  股票节点={len(stocks)} 新闻节点={len(news_ids)} 行业节点={len(industries)}")
    print(f"  SS边={len(ss_edge_src)} NS边={len(ns_edge_src)} IS边={len(is_edge_src)} NI边={len(ni_edge_src)}" )
